...
class DraftsRedraftFailed extends Command
{
    ...
    public function handle(): int
    {
        ...
        $drafts = Draft::query()
            ...
            // NEVER auto-rewrite operator-authored copy. Customised posts
            // (agent_role='operator' / prompt_version='customised-post.v1')
            // carry words the human wrote; a flagged one must surface for
            // manual review/edit, not be silently regenerated by the Writer.
            // agent_role is NOT NULL on every draft, so != is safe (no NULL rows).
            ->where('agent_role', '!=', 'operator')
            ->where('prompt_version', '!=', 'customised-post.v1')
